...
def main():
    ...
    block2 = convlstm_block(block1, 64)
    block3 = convlstm_block(block2, 128)
    block4 = convlstm_block(block3, 256)
    final = ConvLSTM3D(1, (3, 3, 3), padding='same', return_sequences=True)(block4)
    ...
    training_model.fit(X_train, y_train, validation_data=(X_val, y_val), epochs=5, batch_size=1, callbacks=[early_stopping])
