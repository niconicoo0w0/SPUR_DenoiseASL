        ...
        for i in range(data.shape[0]):
            for j in range(data.shape[1]):
                for k in range(data.shape[2]):
                    data[i, j, k, :] = moving_average(data[i, j, k, :], 10)
        mean = np.mean(data, axis=(0, 1, 2), keepdims=True)
        std = np.std(data, axis=(0, 1, 2), keepdims=True)
        data = (data - mean) / (std + 1e-6)
        data = np.transpose(data, (2, 3, 0, 1))
...
def main():
    ...
    for subj in SUBIDS:     
        ...
        save_as_nifti(denoised_image, '/data/GE_MCI/Scripts/DenoiseASL/NicoleTest/GE_MBME_ASL_RS/ASL/BH/Result3/' + subj + '_denoised.nii.gz', orig_path)
