...
def get_nifti_data(subj, DATA_SIZE=DATA_SIZE, OVERLAP=0, target=0):
    ...
    if target == 1:
        nifti_file = nib.load('/data/GE_MCI/Scripts/DenoiseASL/NicoleTest/GE_MBME_ASL_RS/ASL/Checkerboard/' + subj + '_GE_MBME_Checkerboard_ASL_s4_rs35_mean.nii.gz')
    else:
        nifti_file = nib.load('/data/GE_MCI/Scripts/DenoiseASL/NicoleTest/GE_MBME_ASL_RS/ASL/Checkerboard/' + subj + '_GE_MBME_Checkerboard_ASL_s4_rs35.nii.gz')
...
def main():
    ...
    for idx, test_subj in enumerate(SUBIDS):
        ...
        # fix early stopping
        early_stopping = CustomEarlyStopping(monitor='val_loss', patience=2, verbose=1, start_epoch=5)
        # early_stopping = EarlyStopping(monitor='val_loss', patience=2, verbose=1)
        training_model.fit(input_data_train, target_data_train, validation_data=(input_data_test, target_data_test), epochs=15, batch_size=16, callbacks=[early_stopping])
    ...
    for subj in SUBIDS:     
        ...
        orig_path = '/data/GE_MCI/Scripts/DenoiseASL/NicoleTest/GE_MBME_ASL_RS/ASL/Checkerboard/' + subj + '_GE_MBME_Checkerboard_ASL_s4_rs35.nii.gz'
        save_as_nifti(denoised_image, '/data/GE_MCI/Scripts/DenoiseASL/NicoleTest/GE_MBME_ASL_RS/ASL/Checkerboard/Result3/' + subj + '_denoised.nii.gz', orig_path)
