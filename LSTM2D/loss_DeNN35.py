...
def main():
    ...
    training_model.fit(X_train, y_train, validation_data=(X_val, y_val), epochs=10, batch_size=32, callbacks=[early_stopping])
    ...
    for path in test_subjs:     
        ...
        #tsnr before
        mean = np.mean(input_chunk, axis=1)
        std = np.std(input_chunk, axis=1)
        mean[std == 0] = 1e-5
        tsnr_before = mean / std
        ...
        orig_path = '/data/GE_MCI/' + path + '/MNINonLinear/Results/HyperMEPI_PCASL_RS_PA_ASL/HyperMEPI_PCASL_RS_PA_ASL_hp2000_s6_level1.feat/HyperMEPI_PCASL_RS_PA_ASL_hp2000_s6_e1_ss_rs35.nii.gz'
        original_nifti = nib.load(orig_path)
        affine = original_nifti.affine
        save_as_nifti(denoised_image, '/data/GE_MCI/Scripts/DenoiseASL/NicoleTest/nicole/' + path + '_denoised.nii.gz', orig_path)
        ...
        tsnr_after = mean / std
        print("The tsnr before denoising is:", tsnr_before)
        print("The tsnr after denoising is:", tsnr_after)
        
        tsnr_img_before = nib.Nifti1Image(tsnr_before, affine)
        save_as_nifti(tsnr_img_before, '/data/GE_MCI/Scripts/DenoiseASL/NicoleTest/nicole/' + path + '_tsnr_img_after.nii.gz', orig_path)

        tsnr_img_after = nib.Nifti1Image(tsnr_after, affine)
        save_as_nifti(tsnr_img_after, '/data/GE_MCI/Scripts/DenoiseASL/NicoleTest/nicole/' + path + '_tsnr_img_after.nii.gz', orig_path)

        # mean
        mean_img = nib.Nifti1Image(mean, affine)
        save_as_nifti(mean_img, '/data/GE_MCI/Scripts/DenoiseASL/NicoleTest/nicole/' + path + '_mean.nii.gz', orig_path)
